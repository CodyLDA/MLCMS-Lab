...
public class SIRGroupModel extends AbstractGroupModel<SIRGroup> {
	...
	private void initializeGroupsOfInitialPedestrians() {
		...
		if (c.getElements().size() > 0) {
			for(Pedestrian pes : c.getElements()) {
				if(pes.getGroupIds().getFirst() == (SIRType.ID_INFECTED.ordinal()))
					assignToGroup(pes, 0);
				else if (pes.getGroupIds().getFirst() == SIRType.ID_SUSCEPTIBLE.ordinal())
					assignToGroup(pes, 1);
				else if (pes.getGroupIds().getFirst() == SIRType.ID_RECOVERED.ordinal())
					assignToGroup(pes, 3);
			}
		};
		...
		if (c.getElements().size() > 0) {
			...
			for(Pedestrian p : c.getElements()) {
				if (getGroup(p).getID() == SIRType.ID_RECOVERED.ordinal())
					...
					if (this.random.nextDouble() < attributesSIRG.getRecoveryProbability()){
						elementRemoved(p);
					...
					List<Pedestrian> p_neighbors = CellsGrid.getObjects(p.getPosition(), attributesSIRG.getInfectionMaxDistance());
